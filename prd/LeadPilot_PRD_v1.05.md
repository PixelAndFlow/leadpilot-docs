...
**Date:** July 11, 2026
**Status:** Revision of v1.04 — reverses the Google Sheets/Drive access
model from a shared service account to per-rep OAuth (`drive.file`
scope + Google Picker), and adds one new tool. This came out of a Step
0 conversation (see `ai-conversations/claude/2026-07-11-step-0-walkthrough-and-sheet-access-model-pivot.md`):
Marc wants a rep's own Google permissions to determine what LeadPilot
can access — sheets are never pre-shared to a standing service-account
identity — both so the agent can be handed a new sheet "without
notice" without a manual sharing step, and so activity is trackable
per rep at the Google layer, not just inside LeadPilot's own
contact-history log. Three changes from v1.04: (1) `fetch_all_leads`,
`update_lead_sheet`, and `verify_drive_contents` now authenticate as
the requesting rep via OAuth (`drive.file` scope), not a service
account — see Decision 026, which supersedes Decision 024; (2) the
hourly batch run becomes per-rep rather than a single global run — see
Decision 027; (3) a new tool, `fetch_ad_hoc_sheet` (name provisional),
lets a rep point LeadPilot at a sheet mid-session for a one-off read —
see Decision 028. See `prd/LeadPilot_PRD_v1.04.md` for the prior
revision.
...
| `verify_drive_contents` *(auth model changed in v1.05)* | Inspects target Google Drive folder pathways to verify file structures and check file presence, authenticated as the requesting rep via the same `drive.file`/Picker consent as `fetch_all_leads` — not a service account | Google Drive API (`GET /v3/files`), authenticated as the requesting rep | Document names, types (PDF, image), file sizes, metadata, creation timestamps |
