# Reusable Docs Repo Schema — Context File v001
...
| `decisions/` | Numbered decision log: what, why, alternatives rejected, status (Active/Superseded). This is the single highest-value folder for "why does it work this way" and doubles as public/ source material. |
...
| `roadmap/` | Public-facing phase roadmap, written in outcome language (no tool/API names). Feeds `public/`. |
...
| `public/` | Conventions + a sanitization checklist for turning private material into public "build in public" content. The private repo itself is never published; this folder is the curated output layer. |
...
## Expanded `security/` structure (worth doing from day one, not just once a project is defensive-security-mature)
  ...
  dependency-vulnerability-policy.md Scanning cadence, CVE SLA
...
- Should the `public/` folder be scaffolded immediately, or added
  later once there's something worth sharing?
...
- If you find yourself wanting a 20th folder, add it here too (bump to
  v002) so the next project benefits from the same gap analysis
