...
- Decision 024 (Google Sheets service account) needs Marc's explicit
  sign-off — flagged there, not yet confirmed by him
- Whether Gmail-as-the-rep (Step 2, `send_lead_email`) really does need
  the `GOOGLE_OAUTH_CLIENT_ID`/`SECRET` per-rep-consent flow kept in
  `.env.example`, or whether there's a simpler option — not evaluated
  yet, just assumed different from Decision 024's Sheets service
  account
...
- Decisions 001-002 and 006-008 are sourced directly from
  ...
  to avoid blocking on Step 0, 024 explicitly flagged for Marc's
  confirmation; 025 is the dedup/run-lock schema design this file
  previously listed as an open item.
